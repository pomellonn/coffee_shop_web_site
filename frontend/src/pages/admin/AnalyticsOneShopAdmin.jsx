...
import { getShopAnalytics, getAllShops } from "../../services/adminService";
...
const AnalyticsOneShopAdmin = () => {
  ...
  const loadAnalytics = async () => {
    ...
    if (!selectedShopId) {
      alert("Выберите кофейню");
      return;
    }

    setLoading(true);
    try {
      const data = await getShopAnalytics(dateFrom, dateTo, selectedShopId);
      setAnalytics(data);
    } catch (e) {
                    ...
                    {analytics.monthly_summary.map((m) => (
                      ...
                      >
                        ...
                        <span className="text-gray-900 text-sm w-24 text-right">
                          {m.revenue.toFixed(0)} руб.
                        </span>
                        <span className="text-gray-900 text-sm w-24 text-right">
                          {m.avg_check.toFixed(0)} руб.
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  <Bar
                    data={{
                      ...
                      datasets: [
                        ...
                        {
                          label: "Выручка",
                          data: analytics.monthly_summary.map((m) => m.revenue),
                  ...
                  <Bar
                    data={{
                      ...
                      datasets: [
                        {
                          label: "Продано",
                          data: analytics.product_ranking.map(
                            (p) => p.total_sold
                          ),
                          backgroundColor: "rgb(185, 201, 166)",
                        },
...
export default AnalyticsOneShopAdmin;